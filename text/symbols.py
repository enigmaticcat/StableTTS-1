...
# Khmer Unicode characters (used in fallback when words not in lexicon)
_KHMER_UNICODE = [
    'ក', 'ខ', 'ង', 'ច', 'ជ', 'ញ', 'ដ', 'ឋ', 'ណ', 'ត', 'ទ', 'ធ', 'ន', 
    'ប', 'ផ', 'ព', 'ភ', 'ម', 'យ', 'រ', 'ល', 'វ', 'ស', 'ហ', 'អ', 'ឧ',
    'ា', 'ិ', 'ី', 'ុ', 'ើ', 'េ', 'ែ', 'ោ', 'ៅ', 'ំ', 'ះ', '៉', '់', 
    '៍', '័', '្'
]

# _CNM3_letters = []  # Comment out if only training Khmer
...
# Alternative: For minimal Khmer-only (smaller model, but less flexible):
# symbols = [_pad] + list(_punctuation) + _KHMER_IPA + _KHMER_UNICODE + _additional

# For multilingual training (Chinese + Khmer): Uncomment below and comment above
